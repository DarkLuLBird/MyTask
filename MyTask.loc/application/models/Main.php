...
class Main extends Model
{
    ...
    public function tryRegisterUser($post)
    {
        ...
        $message = '
            ...
            <body>
            <p>Что бы подтвердить Email, перейдите по <a href="http://mytask.loc/confirm?hash=' . $hash . '">ссылке</a></p>
            </body>
            </html>
        ';
    ...
    public function forgotPassword($post)
    {
        ...
        $message = '
            ...
            <body>
            <p>Что бы восстановить пароль, перейдите по <a href="http://mytask.loc/recover?hash=' . $hash . '">ссылке</a></p>
            </body>
            </html>
        ';
    ...
    public function createPost($post){
        ...
        $this->image->upload($_FILES);
        $user_id = $_SESSION['authorize']['id'];
        $params = [
            'title' => $title,
            'description' => $description,
            'img_dir' => $this->image->getDir(),
            'user_id' => $user_id,
        ];
        $sql = "INSERT INTO `posts` (`id`, `title`, `description`, `img_dir`, `user_id`) VALUES (NULL, :title, :description, :img_dir, :user_id)";
        $this->db->query($sql, $params);
    }
